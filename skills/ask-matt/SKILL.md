...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out by running **`/two-axis-code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/two-axis-code-review`** on its own whenever you want to review a ticket-tracked branch or PR against its spec — for an ad hoc diff review with no tracked spec, use the repo's general-purpose `/code-review` instead.
...
- **`/deep-research`** — this repo skips Pocock's `research` skill (a thin background-agent-reads-primary-sources tool) in favor of the existing `deep-research` skill, which does the same job with more rigor (multi-source fan-out, adversarial claim verification). Use it to feed research *into* the main flow at `/grill-with-docs`.
